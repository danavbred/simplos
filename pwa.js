...
(function() {
  ...
  function initializeAppDownloadButton() {
    ...
    downloadAppButton.addEventListener('click', async () => {
      if (isIOS) {
        ...
        // If pwaPrompt isn't available but we're not on iOS, show instructions
  ...
  // Initialize the download button when DOM is loaded
  ...
  // Only register service worker if we're on a proper server (not file://)
  if ('serviceWorker' in navigator && (window.location.protocol === 'https:' || window.location.hostname === 'localhost')) {
    ...
    console.log('Service Worker is not supported or we are not on a valid protocol (needs http/https)');
  }
})();